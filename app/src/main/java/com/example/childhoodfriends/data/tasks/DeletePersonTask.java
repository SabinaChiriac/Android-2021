...
public class DeletePersonTask extends AsyncTask<PersonItem,Void,Void> {
    private PersonDatabase personDatabase;
    private PersonRepository.OnSuccesListener listener;

    public DeletePersonTask(PersonDatabase personDatabase,PersonRepository.OnSuccesListener listener){
        this.personDatabase=personDatabase;
        this.listener=listener;
    }
    @Override
    protected Void doInBackground(PersonItem... personItems){
        personDatabase.personDAO().Delete(personItems[0]);
        return  null;
    }
    @Override
